...
logging.basicConfig(filename=f'jexboss_{str(datetime.datetime.now().date())}.log', format=FORMAT, level=logging.INFO)
...
def auto_update():
    ...
    jexboss.print_and_flush(f"{GREEN} * Downloading the new version from {url}.{END}")

    r = gl_http_pool.request('GET', url)
    if r.status != 200:
        jexboss.print_and_flush(
            f"{RED} * Error: Could not complete the download of the new version. Check your internet connection.{END}")

        return False
    with open('master.zip', 'wb') as f:
        f.write(r.data)
    z = ZipFile('master.zip', 'r')
    jexboss.print_and_flush(f"{GREEN} * Extracting new version...{END}")
    z.extractall(path='.')
    z.close()
    os.remove('master.zip')
    path_new_version = f'.{os.path.sep}jexboss-master'
    jexboss.print_and_flush(f"{GREEN} * Replacing the current version with the new version...{END}")
...
def check_updates():
    ...
    try:
        ...
        logging.warning("Failed to check for updates.", traceback)
        return False
    if r.status == 200:
        return _extracted_from_check_updates_19(r)
...
# TODO Rename this here and in `check_updates`
def _extracted_from_check_updates_19(r):
    ...
    if tup(last_version) <= tup(current_version):
        return False
    jexboss.print_and_flush(
        f"{GREEN}{BOLD} * NEW VERSION AVAILABLE: JexBoss v{last_version} ({date_last_version})"
        f"\n{END}{GREEN}   * Link: {link}\n{GREEN}   * Release notes:")
